#include "shashidi.h"  // 包含头文件 shashidi.h
...
void SBaslerCameraControl::UpdateCameraList()  // 更新摄像头列表
{
    ...
    ITransportLayer * pTl = TLFactory.CreateTl("BaslerGigE");  // 创建 BaslerGigE 传输层实例
    ...
    for (int i=0 ; i<cameraArray.GetSize() ; i++) {  // 遍历所有设备
        cameraArray[i].Attach(TLFactory.CreateDevice(devices[i]));  // 附加设备
        string sn = cameraArray[i].GetDeviceInfo().GetSerialNumber();  // 获取设备序列号
        m_cameralist << QString::fromStdString(sn);  // 将序列号添加到摄像头列表
    }
