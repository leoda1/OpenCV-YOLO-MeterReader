...
static inline double v2ang(double v, double vmax, double startDeg, double totalAngle,QList<double> points,QList<double> pointsAngle){
    //  确保两端点完整（若已经在列表里则不会重复添加）
    QList<double> allP = points;
    QList<double> allA = pointsAngle;
...
QImage DialMarkDialog::generateBYQDialImage()
{
    ...
    const QList<double>& points = m_byqConfig.points;           // 保存的分段点
    const QList<double>& pointsAngle = m_byqConfig.pointsAngle; // 保存的分段角度
...
void DialMarkDialog::drawBYQTicksAndNumbers(QPainter& p, const QPointF& C, double outerR,
    double startDeg,double totalAngle, double spanDeg, double vmax, double majorStep,QList<double> points,QList<double> pointsAngle)
{
...
void DialMarkDialog::drawBYQColorBands(QPainter& p, const QPointF& C, double outerR,
    double startDeg,double totalAngle, double spanDeg, double vmax,QList<double> points,QList<double> pointsAngle)
{
    const double r17_1 = outerR;   // 彩带外沿
    const double r16_1 = outerR * (16.1 / 17.1);   // 彩带内沿
...
QPixmap DialMarkDialog::generateYYQYDialImage()
{
    ...
    const QList<double>& points = m_yyqyConfig.points;           // 保存的分段点
    const QList<double>& pointsAngle = m_yyqyConfig.pointsAngle; // 保存的分段角度
...
//实在不行就yyqy2ang
static inline double yyqyV2Ang(double v, double vmax, double startDeg, double totalAngle,QList<double> points,QList<double> pointsAngle)
{
    //  确保两端点完整（若已经在列表里则不会重复添加）
    QList<double> allP = points;
    QList<double> allA = pointsAngle;
...
void DialMarkDialog::addBYQconfig(double maxPressure,double totalAngle,QList<double> points,QList<double> pointsAngle) 
{
...
void DialMarkDialog::addYYQYconfig(double maxPressure,double totalAngle,QList<double> points,QList<double> pointsAngle)
{
